...
public:
   ...
   CBullishHarami*   GetBullishHarami() { return m_bullishHarami; }
   CBearishHarami*   GetBearishHarami() { return m_bearishHarami; }
   CHaramiCross*     GetHaramiCross() { return m_haramiCross; }
};
...
bool CHaramiPatterns::IsValidPointer(void* ptr)
{
...
bool CHaramiPatterns::Initialize(string symbol = "", ENUM_TIMEFRAMES timeframe = PERIOD_CURRENT)
{
   ...
   // تهيئة جميع الأنماط
   bool success = true;
   if(IsValidPointer(m_bullishHarami)) 
      success = success && m_bullishHarami.Initialize(symbol, timeframe);
   else
      success = false;
      
   if(IsValidPointer(m_bearishHarami)) 
      success = success && m_bearishHarami.Initialize(symbol, timeframe);
   else
      success = false;
      
   if(IsValidPointer(m_haramiCross)) 
      success = success && m_haramiCross.Initialize(symbol, timeframe);
   else
      success = false;
...
void CHaramiPatterns::Deinitialize()
{
   if(IsValidPointer(m_bullishHarami)) 
   { 
      delete m_bullishHarami; 
      m_bullishHarami = NULL; 
   }
   
   if(IsValidPointer(m_bearishHarami)) 
   { 
      delete m_bearishHarami; 
      m_bearishHarami = NULL; 
   }
   
   if(IsValidPointer(m_haramiCross)) 
   { 
      delete m_haramiCross; 
      m_haramiCross = NULL; 
   }
                                     ...
                                     SPatternDetectionResult &results[])
{
   ...
   // كشف الحامل الصعودي
   if(m_enableBullish && IsValidPointer(m_bullishHarami))
   {
   ...
   // كشف الحامل الهبوطي
   if(m_enableBearish && IsValidPointer(m_bearishHarami))
   {
   ...
   // كشف صليب الحامل
   if(m_enableCross && IsValidPointer(m_haramiCross))
   {
