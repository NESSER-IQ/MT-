...
bool CTweezerPatterns::Initialize(string symbol = "", ENUM_TIMEFRAMES timeframe = PERIOD_CURRENT)
{
   ...
   // تهيئة جميع الأنماط
   bool success = true;
   if(m_tweezerTop) success &= m_tweezerTop.Initialize(symbol, timeframe);
   if(m_tweezerBottom) success &= m_tweezerBottom.Initialize(symbol, timeframe);
...
{
   if(m_tweezerTop) { delete m_tweezerTop; m_tweezerTop = NULL; }
   if(m_tweezerBottom) { delete m_tweezerBottom; m_tweezerBottom = NULL; }
                                      ...
                                      SPatternDetectionResult &results[])
{
   ...
   // كشف قاع الملقاط
   if(m_enableBottom && m_tweezerBottom)
   {
