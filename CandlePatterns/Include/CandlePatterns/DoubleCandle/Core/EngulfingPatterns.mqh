                                               ...
                                               const double &low[], const double &close[], const long &volume[])
{
   ...
   if(m_volumeThreshold > 0 && ArraySize(volume) > idx)
   {
...
bool CEngulfingPatterns::Initialize(string symbol = "", ENUM_TIMEFRAMES timeframe = PERIOD_CURRENT)
{
   ...
   // تهيئة جميع الأنماط
   bool success = true;
   if(m_bullishEngulfing) success &= m_bullishEngulfing.Initialize(symbol, timeframe);
   if(m_bearishEngulfing) success &= m_bearishEngulfing.Initialize(symbol, timeframe);
   if(m_partialEngulfing) success &= m_partialEngulfing.Initialize(symbol, timeframe);
...
{
   if(m_bullishEngulfing) { delete m_bullishEngulfing; m_bullishEngulfing = NULL; }
   if(m_bearishEngulfing) { delete m_bearishEngulfing; m_bearishEngulfing = NULL; }
   if(m_partialEngulfing) { delete m_partialEngulfing; m_partialEngulfing = NULL; }
                                        ...
                                        SPatternDetectionResult &results[])
{
   ...
   if(m_enableBullish && m_bullishEngulfing)
   {
   ...
   if(m_enableBearish && m_bearishEngulfing)
   {
   ...
   // كشف الابتلاع الجزئي
   if(m_enablePartial && m_partialEngulfing)
   {
