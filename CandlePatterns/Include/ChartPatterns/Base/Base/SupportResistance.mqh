...
public:
   ...
   // تحليل الحجم - النسخة المُصححة
   bool              HasVolumeConfirmation(const SAdvancedSRLevel &level, const long &volume[],
                                         const int startIdx, const int endIdx);
   
   // نسختان من CalculateVolumeAtLevel - إصلاح مشكلة NULL reference
   ...
   double            CalculateVolumeAtLevelNoPrice(const SAdvancedSRLevel &level, 
                                                 const long &volume[], const int startIdx, const int endIdx);
...
//+------------------------------------------------------------------+
//| فحص تأكيد الحجم - النسخة المُصححة                                |
//+------------------------------------------------------------------+
bool CSupportResistance::HasVolumeConfirmation(const SAdvancedSRLevel &level, const long &volume[],
                                              const int startIdx, const int endIdx)
{
   // حساب متوسط الحجم عند هذا المستوى - استخدام النسخة بدون أسعار
   double levelVolume = CalculateVolumeAtLevelNoPrice(level, volume, startIdx, endIdx);
...
//+------------------------------------------------------------------+
//| حساب الحجم عند المستوى - مع بيانات الأسعار                      |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| حساب الحجم عند المستوى - بدون بيانات أسعار (إصلاح مشكلة NULL)   |
//+------------------------------------------------------------------+
double CSupportResistance::CalculateVolumeAtLevelNoPrice(const SAdvancedSRLevel &level, 
                                                        const long &volume[], const int startIdx, const int endIdx)
{
   long totalVolume = 0;
   int count = 0;
